    ...
    <div class="row h-100 justify-content-center">
      <div class="col-9 col-sm-10 col-md-6 col-lg-6 mt-5">
        ...
        <div class="row mt-4">
          ...
          <h6 class="mr-4"><a href="" @click.prevent="getArchived">Archive</a></h6>
          <h6 class="add-btn">
            <a @click.prevent="isAddNote = !isAddNote" href="">{{isAddNote ? 'View Notes' : 'Add Note'}}</a>
          </h6>
        </div>
        ...
        <div class="row mt-3" v-if="isAddNote">
          ...
          <div class="mr-2 mb-4">
            ...
            <button class="btn btn-sm btn-login mt-4" @click.prevent="saveNote">
              {{
                status == "save"
                  ? isLoading
                    ? "Saving ..."
...
export default {
  ...
  methods: {
    ...
    async saveNote() {
      if (this.status == "save") {
        const res = await this.callApi("post", `api/note/store?token=${this.$store.state.token}`, this.data);
        if (res.status == 200) {
        ...
        this.isLoading = true;
        const res = await this.callApi("put", `api/note/put?token=${this.$store.state.token}`, this.data);
        if (res.status == 200) {
    ...
    async getNotes(page = 1) {
      const res = await this.callApi("get", `api/note/get?token=${this.$store.state.token}&&page=${page}&&total=${this.total}`);
      if (res.status == 200) {
        this.notes = res.data;
    ...
    async deleteNote() {
      const res = await this.callApi("delete", `api/note/delete?token=${this.$store.state.token}&&id=${this.deleteid}`);
      if (res.status == 200) {
    ...
    async getArchived(page = 1){
      ...
      this.status = "save";
      this.isAddNote = false
      const res = await this.callApi('get', `api/note/getArchived?token=${this.$store.state.token}&&page=${page}&&total=${this.total}`)
      if (res.status == 200) {
        this.notes = res.data;
      }
    },
    async archiveNote(){
      const res = await this.callApi('put', `api/note/addArchived?token=${this.$store.state.token}&&id=${this.archiveid}`)
      if (res.status == 200) {
        this.notes = res.data;
        this.getNotes()
    ...
    async unarchiveNote(){
      const res = await this.callApi('put', `api/note/unarchive?token=${this.$store.state.token}&&id=${this.archiveid}`)
      if (res.status == 200) {
    ...
    async logout() {
      const res = await this.callApi('post', 'api/auth/logout', {token: this.$store.state.token})
      if(res.status == 200){
